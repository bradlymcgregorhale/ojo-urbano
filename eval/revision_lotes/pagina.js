...
function render(){if(D.reanalisis)setTimeout(refrescarReanalisis,0);const id=state.actual,p=D.fotos.find(p=>p.foto===id),a=original(id),saved=state.revisiones[id];draft=saved?structuredClone(saved):suggested(id);$('foto').src=p.archivo;$('foto').alt='Foto '+id;$('foto-link').href=p.entrada_api||p.archivo;$('titulo').textContent=id;$('foto-select').value=id;$('resultado-pendiente').hidden=!!a;$('resultado').hidden=!a;$('acciones').hidden=!a;$('atajo-interior').hidden=!a;$('editor').hidden=true;$('mensaje').textContent='';$('revision-estado').textContent=saved?(D.resultados[id]?.huella!==saved.original_huella?'Respuesta modificada: revisar de nuevo':saved.estado):'Pendiente';$('anterior').disabled=ids.indexOf(id)===0;$('siguiente').disabled=ids.indexOf(id)===ids.length-1;if(a){$('decision-original').textContent=a.hay_reclamo?'Confirma reclamo':(a.posibles||[]).length?'No confirma; informa posibles':(a.en_duda||[]).length?'No confirma; quedan categorías en duda':'No confirma reclamo';$('estado-original').textContent='Modo: Completo · Estado: '+a.analisis_estado+' · USD '+a.costo_api+' · '+(a.tokens_api??'Sin conteo')+' tokens'+(a.tokens_api_completos===false?' (conteo incompleto)':'');$('descripcion-original').textContent=a.descripcion||'Sin explicación';$('json-original').textContent=JSON.stringify(a,null,2);const box=$('categorias-originales');box.replaceChildren();for(const[field,title]of [['problemas','Confirmados'],['posibles','Posibles'],['elementos_detectados','Elementos'],['en_duda','En duda']]){const h=document.createElement('h4');h.textContent=title;box.append(h);const ul=document.createElement('ul');for(const v of a[field]||[]){const li=document.createElement('li');li.textContent=typeof v==='string'?(cats[v]?.nombre||v):v.nombre||cats[v.key]?.nombre||v.key||v.codigo;ul.append(li);}if(!ul.children.length){const li=document.createElement('li');li.textContent='Ninguno';ul.append(li);}box.append(ul);}$('parcial-label').hidden=a.analisis_estado==='completo';fillEditor();}updateProgress();}
