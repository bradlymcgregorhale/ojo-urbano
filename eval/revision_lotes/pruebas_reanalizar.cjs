...
(async()=>{
 ...
 const cacheLedger=path.join(retryBase,'analisis/reanalisis/estado.json'),cl=JSON.parse(fs.readFileSync(cacheLedger));cl.activo=sameId;cl.intentos.find(a=>a.id===sameId).estado='preparado';fs.writeFileSync(cacheLedger,JSON.stringify(cl));
 const cacheRecovery=crearServicio(retryBase,config,{enviar:async()=>{throw Error('No repetir caché guardada')}});await termina(cacheRecovery);assert.equal(cacheRecovery.estado().activo,null);assert.equal(cacheRecovery.estado().reserva_incierta_usd,0);assert.equal(cacheRecovery.estado().gasto_usd,0);
 ...
 const jobFailureBase=preparar();let posts=0;const jobFailure=crearServicio(jobFailureBase,config,{version:async()=>'nueva',enviar:async()=>{posts++;return{trabajo:'known',estado:'en_cola'}},consultar:async()=>({estado:'error'})});jobFailure.iniciar('H0001',crypto.randomUUID());await wait(40);assert.equal(jobFailure.estado().reserva_incierta_usd,1);
 const jobRecovered=crearServicio(jobFailureBase,config,{consultar:async()=>({estado:'listo',resultado:r}),enviar:async()=>{posts++;throw Error('No repetir POST')}});await termina(jobRecovered);assert.equal(posts,1);assert.equal(jobRecovered.estado().reserva_incierta_usd,0);assert.equal(jobRecovered.estado().gasto_usd,.020001);assert.equal(jobRecovered.estado().bloqueo,null);
