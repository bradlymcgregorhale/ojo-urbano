...
(async()=>{
 ...
 const b=await pp.launch({headless:true,protocolTimeout:15000});try{
  ...
  console.log('duplicado controlado');await next.bringToFront();const cdp=await next.createCDPSession();await cdp.send('Page.setDownloadBehavior',{behavior:'allow',downloadPath:base});await next.click('#exportar');
