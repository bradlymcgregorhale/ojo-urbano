...
Se generan un HTML y un JSON con aciertos conservados, regresiones, correcciones y errores persistentes. Sin `--candidata`, mide la referencia contra las etiquetas humanas. La comparación no llama a modelos ni consulta la red. Una ausencia en la respuesta equivale a `no` solo para las categorías que una persona revisó expresamente. Las categorías posibles se distinguen de las confirmadas. `duda` y `sin_revisar` no se puntúan. Las revisiones contradictorias bloquean la comparación, sin elegir una por fecha. En los interiores no se crean negativos de cada categoría.
...
Salida 0 significa que se conservaron los aciertos previos evaluados sin falta de cobertura ni conflictos. Salida 1 señala regresiones, datos faltantes, conflictos o ausencia de aciertos previos para proteger. Salida 2 señala archivos inválidos. Ninguna salida aprueba por sí sola un despliegue ni demuestra exactitud general. Un registro compuesto solo por correcciones de errores necesita también aprobaciones humanas de casos correctos para probar preservación.
