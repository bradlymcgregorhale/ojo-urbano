...
(async()=>{
 ...
 assert.equal(await p.evaluate(()=>document.characterSet),'UTF-8');assert.equal(await p.$eval('h1',e=>e.textContent),'¿Qué ves en esta foto?');assert(await p.$eval('#legacy',e=>e.hidden));assert.equal(await p.$eval('#foto-select',e=>e.value),'X001');assert.equal(await p.$$eval('#form-revision input:checked',a=>a.length),0);
 ...
 await p.click('#revelar');const cdp=await p.createCDPSession();await cdp.send('Page.setDownloadBehavior',{behavior:'allow',downloadPath:downloads});await p.click('#exportar');
 let exported;for(let n=0;n<40;n++){exported=fs.readdirSync(downloads).filter(f=>f.endsWith('.json')).sort().pop();if(exported)break;await new Promise(r=>setTimeout(r,50));}assert(exported);exported=path.join(downloads,exported);const saved=JSON.parse(fs.readFileSync(exported));assert(saved.revisiones.X001.completa);await(await p.$('#archivo-revision')).uploadFile(exported);await p.waitForSelector('#import-preview:not([hidden])');assert((await p.$eval('#import-preview',e=>e.textContent)).includes('0 tienen diferencias'));await p.$$eval('#import-preview button',bs=>bs.find(b=>b.textContent==='Cancelar').click());
 const ctx=await b.createBrowserContext(),p2=await ctx.newPage();p2.on('pageerror',e=>errors.push(e.message));await p2.goto('file://'+base+'/entrega/comparacion.html');await (await p2.$('#archivo-revision')).uploadFile(exported);await p2.waitForSelector('#import-preview:not([hidden])');await p2.$eval('#import-preview button',e=>e.click());assert.equal(await p2.$eval('#avance',e=>e.textContent),'1 / 100');
