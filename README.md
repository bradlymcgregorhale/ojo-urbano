...
- `problemas`: lo que se reporta, con gravedad 1-5 y `fuentes`: **cuántas** fuentes del consenso lo sostienen (un conteo; hacen falta al menos 2). El clasificador interno participa del consenso como una fuente más, pero su voto individual no se publica. Si la foto no corresponde al reclamo, acá va lo que pidió el vecino (una sola fuente: su texto). La entrada puede traer `codigo` (una prestación del catálogo completo de la Ciudad) **en lugar de** `key`: leé `p.get("key") or p.get("codigo")`, nunca `p["key"]` a secas. Cuando la escena apunta a `vehiculo_mal_estacionado` o `vehiculo_abandonado` — porque algún modelo de visión lo reporta, el `contexto` del vecino lo señala, o el clasificador interno lo sospecha; confirmado o no: el dato sirve aunque la infracción no se confirme desde la foto — la respuesta puede traer la patente del vehículo: top-level `patente` (formatos argentinos, p. ej. `AB123CD` o `ABC123`), y además copiada en la entrada de `problemas` si el problema quedó confirmado. Aparece **solo** cuando dos lectores independientes leyeron la misma cadena en la chapa del vehículo protagonista y la lectura matchea un formato completo — cualquier lectura válida discrepante, aunque pierda 2 a 1, la suprime. Si hace falta, el sistema relee la foto a mayor resolución solo para la chapa (dos llamadas extra, únicamente en fotos con vehículo reportado). Si la patente no está a la vista o hay cualquier duda, el campo directamente no viene. Campos aditivos, sin cambio de contrato. Cada entrada de `problemas` trae además `confianza` (`alta` = 3 o más fuentes, `media` = 2; lo de una sola fuente ya vive en `posibles`, que es el nivel bajo del contrato), y la respuesta trae top-level `predominante`: la clave del problema que domina la escena (mayor gravedad; a igual gravedad, más fuentes), `null` sin problemas confirmados. Para reportar "solo lo principal", usá `predominante` o filtrá por gravedad. También aditivos. Excepción única al silencio del clasificador interno: los **escombros embolsados**, que los modelos de visión generalistas no distinguen de bolsas comunes (medido: 7 modelos, 0 detecciones en fotos nocturnas) y el clasificador interno sí. La fusión corre **solo con la verificación cruzada activa** (necesita que los verificadores corroboren la pila) y tiene **dos niveles**: uno CONFIADO (interno ≥0.95 para escombros y recolección local ≤0.2) y uno de RESCATE, más estricto en la ambivalencia (interno ≥0.70 y recolección local ≤0.1), para las bolsas de cascote que el interno puntúa en la banda media. En ambos hace falta además que en `problemas` haya una pila confirmada aparte, `recoleccion` o `retiro_muebles` (no se comprueba que sea físicamente la misma pila). **Guardas:** la poda confirmada veta los dos niveles; el rechazo dirigido de un verificador ("no son escombros") también los veta, salvo en el nivel confiado cuando la corroboración viene sólo de `retiro_muebles` (sin `recoleccion`). Cuando se dispara y `retiro_escombros` no venía ya confirmado, la entrada entra a `problemas` marcada con `reclasificado_por: "modelo_local"`; y si la recolección estaba confirmada y su score local es bajo (≤ `FUSION_ESCOMBROS_RECO_BAJA`), la entrada `recoleccion` baja a `posibles` con su motivo. Umbrales por defecto, configurables: `FUSION_ESCOMBROS` (se apaga con `0`, `false` o `no`), `FUSION_ESCOMBROS_UMBRAL` (0.95) / `FUSION_ESCOMBROS_RECO_BAJA` (0.2) para el nivel confiado y `FUSION_ESCOMBROS_UMBRAL_RESCATE` (0.70) / `FUSION_ESCOMBROS_RECO_RESCATE` (0.1) para el rescate.
...
- `calidad_foto`: legibilidad de la imagen: `lado_menor` (el lado más corto en píxeles), `nitidez` (varianza del laplaciano normalizada por contraste, medida siempre a lado menor 480 para poder comparar fotos de distinto tamaño), `luminancia` y `definicion` (`buena` / `limitada`). **Es informativo: no filtra, no degrada y no cambia ninguna decisión.** Y es a propósito: sobre 200 fotos con etiqueta humana, la calidad global NO predice los errores. Los falsos positivos por predicción dan 5,6 % en las fotos de hasta 480 px y 9,2 % en las de más de 900, y una compuerta por nitidez en 0,30 habría tocado 82 aciertos para evitar 6 errores. Lo que falla en una foto ilegible falla con los modelos convencidos y unánimes, y eso lo baja una mirada dirigida, no un umbral. Sirve para triar un lote y para revisar el criterio cuando haya etiquetas nuevas.
...
Un contenedor publicado es un dato duro (el sistema del que salen estos reportes tiene los contenedores mapeados), así que inventarlo es caro. Hay dos vetos de presencia, los dos por mirada dirigida y los dos con mayoría, no con un umbral solo:

- **Ninguno**: los verificadores confirman un contenedor y el clasificador interno está en el piso para **todas** las claves de contenedor. Se pregunta dirigido si hay alguno; mayoría de "ausente" y la presencia baja a `en_duda`. Acá también decide la proporción: el contenedor municipal es **ancho** (unos dos metros); un tacho angosto y vertical no lo es, por más grande que se vea.
- **Ese**: el contenedor fantasma que se publica **al lado de uno real** (la bolsa verde leída como contenedor de reciclables). Ahí el veto anterior no salta nunca, porque el interno puntúa altísimo por el contenedor que sí está. Corre por clave, y solo donde el interno demostró ser detector confiable: medido sobre 200 fotos etiquetadas, `contenedor_secos` tiene 30 positivos reales con mínimo 0,427 contra 3 falsos por debajo de 0,005, y el bilateral 39 positivos con mínimo 0,290 contra 2 falsos bajo 0,007. Para el **lateral no se aplica**: es el tipo más común y el que el interno más se pierde (10 de sus 110 positivos reales caen bajo el piso). Cuando se veta, la prosa pierde solo las frases de **ese** contenedor: el real de al lado se sigue describiendo.
