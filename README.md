...
- `problemas`: lo que se reporta, con gravedad 1-5 y `fuentes`: **cuántas** fuentes del consenso lo sostienen (un conteo; hacen falta al menos 2). El clasificador interno participa del consenso como una fuente más, pero su voto individual no se publica. Si la foto no corresponde al reclamo, acá va lo que pidió el vecino (una sola fuente: su texto). La entrada puede traer `codigo` (una prestación del catálogo completo de la Ciudad) **en lugar de** `key`: leé `p.get("key") or p.get("codigo")`, nunca `p["key"]` a secas. En `vehiculo_mal_estacionado` y `vehiculo_abandonado` la entrada puede traer `patente` (formatos argentinos, p. ej. `AB123CD` o `ABC123`): aparece **solo** cuando al menos dos verificadores leyeron la misma cadena en la chapa del vehículo infractor y la lectura matchea un formato completo — cualquier lectura discrepante, aunque pierda 2 a 1, la suprime. Si hace falta, el sistema relee la foto a mayor resolución solo para la chapa (dos llamadas extra, únicamente en fotos con vehículo confirmado). Si la patente no está a la vista o hay cualquier duda, el campo directamente no viene. Campo aditivo, sin cambio de contrato.
