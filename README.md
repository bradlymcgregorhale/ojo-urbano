...
- `evaluacion_foto.requiere_revision`: si hay una observación negativa de ubicación, calidad o encuadre sin corroboración, devuelve `true` junto con una `indicacion`. Conserva los hallazgos y no rechaza automáticamente ni exige otra foto por ese único voto. También indica revisión cuando la calidad contradice problemas corroborados. La falta de evaluación, por sí sola, no activa esta bandera ni demuestra que la foto sea válida.
