...
- `evaluacion_foto.requiere_revision`: devuelve `true` cuando el estado es `senal_negativa_no_corroborada` o `calidad_contradictoria`. Conserva los hallazgos. Si hay un rechazo o un encuadre insuficiente corroborado, prevalece ese estado. La falta de evaluación, por sí sola, no activa esta bandera ni demuestra que la foto sea válida. La `indicacion` puede ser una nota para quien revisa; para pedir otra foto, usá `requiere_nueva_foto` o `requiere_foto_complementaria`, que no se activan por una señal aislada.
