...
- `problemas`: lo que se reporta, con gravedad 1-5 y las `fuentes` que lo vieron. Normalmente son fuentes visuales (modelo local y/o modelos de visión) y hacen falta **al menos 2**. Si la foto no corresponde al reclamo, acá va lo que pidió el vecino, con `fuentes: ["contexto_vecinal"]`. En ese caso la entrada puede traer `codigo` (una prestación del catálogo completo de la Ciudad) **en lugar de** `key`: leé `p.get("key") or p.get("codigo")`, nunca `p["key"]` a secas.
